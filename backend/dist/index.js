...
import { addMessage, ensureDirectChat, findUserById, listChatsForUser, loginUser, messagesForChat, registerUser, searchUsers, startCall, updateCall, } from "./store.js";
...
app.post("/api/messages/direct", requireAuth, (req, res) => {
    const { chatId, text, type, fileUrl, fileName, durationMs } = req.body || {};
    if (!chatId || typeof text !== "string")
        return res.status(400).json({ error: "INVALID_PAYLOAD" });
    const message = addMessage({ chatId: String(chatId), senderId: req.auth.userId, text, type: (type || "text"), fileUrl, fileName, durationMs });
...
app.post("/api/calls/offer", requireAuth, (req, res) => {
    const { chatId, toUserId, mode, sdp } = req.body || {};
    if (!chatId || !toUserId || !mode)
        return res.status(400).json({ error: "INVALID_PAYLOAD" });
...
app.post("/api/calls/answer", requireAuth, (req, res) => {
...
app.post("/api/calls/ice", requireAuth, (req, res) => {
    const { toUserId, callId, candidate } = req.body || {};
    if (!toUserId || !callId || !candidate)
        return res.status(400).json({ error: "INVALID_PAYLOAD" });
